    ...
    public class PetDetailsViewModel : ViewModelBase
    {
        ...
        private async Task OnLoveTapped()
        {
            ...
            if (Pet.UserInteraction.PetLoved == true)
            {
                ...
                await _dialogService.MessageDialogAsync(App.Current.MainPage, "Favorit odabran", "Dodano u vašu listu favorita!");
            }
            else
            {
                var storedPet = favouritePets.Find(x => x.PetID == Pet.PetID);
                favouritePets.Remove(storedPet);
                await _dialogService.MessageDialogAsync(App.Current.MainPage, "Favorit uklonjen", "Uklonjen iz vaše liste favorita!");
            }
